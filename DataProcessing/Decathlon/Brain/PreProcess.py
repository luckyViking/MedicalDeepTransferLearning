...
for i in tqdm(range(len(FolderContentList))):
    ...
    if i == int(len(FolderContentList)/2):
        ...
        #np.save(os.path.join(OutputPath, 'x_train_1.npy'), x_train)
        #np.save(os.path.join(OutputPath, 'y_train_1.npy'), y_train)
        print('Saving first part of x_train and y_train')
        x_train = []
        y_train = []
...
x_train = np.asarray(x_train)
y_train = np.asarray(y_train)
np.save(os.path.join(OutputPath, 'x_train_2.npy'), x_train)
np.save(os.path.join(OutputPath, 'y_train_2.npy'), y_train)
